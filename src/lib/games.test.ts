...
describe('games data-access helpers', () => {
    ...
    it('filters by any selected category', async () => {
        ...
        const [publisher] = await db
            ...
            .returning({ id: publishers.id });
        await db.insert(games).values([
            { title: 'Strategy Game', description: 'one', starRating: 4, categoryId: strategy.id, publisherId: publisher.id },
            { title: 'Puzzle Game', description: 'two', starRating: 4, categoryId: categoryRows[1].id, publisherId: publisher.id },
        ]);

        const filtered = await getFilteredGames(db, { categoryIds: categoryRows.map(({ id }) => id) });
